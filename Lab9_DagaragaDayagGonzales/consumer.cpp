...
int main(int argc, char* argv[]) {
    ...
    // Link to existing IPC Resources (ought to be initialized by Producer first)
    ...
    while (running) {
        ...
        // Producer.out can't read/edit at this point
        semLock(semId);
        ...
        // Skip Calculation (Bonus)
        if (lastSeqNum != 0) {
            // Reset skip count if the video loops
            if (current < lastFrameIdx) {
                skippedFrames = 0; 
            }
            // If sequence jumped, calculate how many frames missed
            if (seq > lastSeqNum + 1) {
